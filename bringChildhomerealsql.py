...
def validPickUp(sCardId, dateToday,type):
    # check wherther the card punch card today or not if return any row then yes
    conn = connectSql()
...
def getEmail(pCardId):
    ...
    try:
        ...
        cursor.close()
def getAttendance(studentid,datetoday):
    conn = connectSql()
...
# valid pick up is string
                validPickUp2 = (int(validPickUp(myFirstCardId, today,"pickUp"))) + (int(validPickUp(myFirstCardId, today,"qrPickUp")))
                ...
                if index == 0 and parentCardStatus == "Valid":
                    ...
                    checkPickUp = (int(validPickUp(Card2id, today,"pickUp"))) + (int(validPickUp(Card2id, today,"qrPickUp")))
                    # check if student picked up already or not
                    AttendanceStatus = getAttendance(Card2id,today)
                    # print(Card2id,"card2")
                    # print(newA[1],"new a [1]")
                    if str(Card2id) == newA[1] and studentCardStatus == "Valid" and int(checkPickUp) < 1 and AttendanceStatus == "present":
                        ...
                        refresh("-----------", "student card havent", "attend today", "-------------")
                    ...
                    AttendanceStatus = getAttendance(myFirstCardId,today)
                    if(AttendanceStatus == "present"):
                        ...
                        refresh("-----------", "student havent attend", "today", "-------------")
                        ledLightOnRed()
                ...
                refresh("-----------------", "No card registerd", "Please register first", "---------------")
